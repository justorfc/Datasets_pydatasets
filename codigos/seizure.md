```python
# Código Colab para dataset: seizure
...
try:
    try:
        df = pd.read_csv('data/seizure.csv')
        print('Cargado desde data/seizure.csv')
    except Exception:
        df = pd.read_excel('data/seizure.xlsx', engine='openpyxl')
        print('Cargado desde data/seizure.xlsx')
except Exception:
    try:
