```python
# Código Colab para dataset: Tuna
...
try:
    try:
        df = pd.read_csv('data/Tuna.csv')
        print('Cargado desde data/Tuna.csv')
    except Exception:
        df = pd.read_excel('data/Tuna.xlsx', engine='openpyxl')
        print('Cargado desde data/Tuna.xlsx')
except Exception:
    try:
