```python
# Código Colab para dataset: stddiag
...
try:
    try:
        df = pd.read_csv('data/stddiag.csv')
        print('Cargado desde data/stddiag.csv')
    except Exception:
        df = pd.read_excel('data/stddiag.xlsx', engine='openpyxl')
        print('Cargado desde data/stddiag.xlsx')
except Exception:
    try:
