```python
# Código Colab para dataset: quine
...
try:
    try:
        df = pd.read_csv('data/quine.csv')
        print('Cargado desde data/quine.csv')
    except Exception:
        df = pd.read_excel('data/quine.xlsx', engine='openpyxl')
        print('Cargado desde data/quine.xlsx')
except Exception:
    try:
