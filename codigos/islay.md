```python
# Código Colab para dataset: islay
...
try:
    try:
        df = pd.read_csv('data/islay.csv')
        print('Cargado desde data/islay.csv')
    except Exception:
        df = pd.read_excel('data/islay.xlsx', engine='openpyxl')
        print('Cargado desde data/islay.xlsx')
except Exception:
    try:
