```python
# Código Colab para dataset: MCAS
...
try:
    try:
        df = pd.read_csv('data/MCAS.csv')
        print('Cargado desde data/MCAS.csv')
    except Exception:
        df = pd.read_excel('data/MCAS.xlsx', engine='openpyxl')
        print('Cargado desde data/MCAS.xlsx')
except Exception:
    try:
