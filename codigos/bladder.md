```python
# Código Colab para dataset: bladder
...
try:
    try:
        df = pd.read_csv('data/bladder.csv')
        print('Cargado desde data/bladder.csv')
    except Exception:
        df = pd.read_excel('data/bladder.xlsx', engine='openpyxl')
        print('Cargado desde data/bladder.xlsx')
except Exception:
    try:
