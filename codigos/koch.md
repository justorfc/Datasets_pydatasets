```python
# Código Colab para dataset: koch
...
try:
    try:
        df = pd.read_csv('data/koch.csv')
        print('Cargado desde data/koch.csv')
    except Exception:
        df = pd.read_excel('data/koch.xlsx', engine='openpyxl')
        print('Cargado desde data/koch.xlsx')
except Exception:
    try:
