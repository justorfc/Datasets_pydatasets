```python
# Código Colab para dataset: wavesurge
...
try:
    try:
        df = pd.read_csv('data/wavesurge.csv')
        print('Cargado desde data/wavesurge.csv')
    except Exception:
        df = pd.read_excel('data/wavesurge.xlsx', engine='openpyxl')
        print('Cargado desde data/wavesurge.xlsx')
except Exception:
    try:
