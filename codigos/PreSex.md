```python
# Código Colab para dataset: PreSex
...
try:
    try:
        df = pd.read_csv('data/PreSex.csv')
        print('Cargado desde data/PreSex.csv')
    except Exception:
        df = pd.read_excel('data/PreSex.xlsx', engine='openpyxl')
        print('Cargado desde data/PreSex.xlsx')
except Exception:
    try:
