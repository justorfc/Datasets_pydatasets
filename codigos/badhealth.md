```python
# Código Colab para dataset: badhealth
...
try:
    try:
        df = pd.read_csv('data/badhealth.csv')
        print('Cargado desde data/badhealth.csv')
    except Exception:
        df = pd.read_excel('data/badhealth.xlsx', engine='openpyxl')
        print('Cargado desde data/badhealth.xlsx')
except Exception:
    try:
