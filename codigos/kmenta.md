```python
# Código Colab para dataset: kmenta
...
try:
    try:
        df = pd.read_csv('data/kmenta.csv')
        print('Cargado desde data/kmenta.csv')
    except Exception:
        df = pd.read_excel('data/kmenta.xlsx', engine='openpyxl')
        print('Cargado desde data/kmenta.xlsx')
except Exception:
    try:
