```python
# Código Colab para dataset: pulpfiber
...
try:
    try:
        df = pd.read_csv('data/pulpfiber.csv')
        print('Cargado desde data/pulpfiber.csv')
    except Exception:
        df = pd.read_excel('data/pulpfiber.xlsx', engine='openpyxl')
        print('Cargado desde data/pulpfiber.xlsx')
except Exception:
    try:
