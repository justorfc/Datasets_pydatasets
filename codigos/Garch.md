```python
# Código Colab para dataset: Garch
...
try:
    try:
        df = pd.read_csv('data/Garch.csv')
        print('Cargado desde data/Garch.csv')
    except Exception:
        df = pd.read_excel('data/Garch.xlsx', engine='openpyxl')
        print('Cargado desde data/Garch.xlsx')
except Exception:
    try:
