```python
# Código Colab para dataset: Cigar
...
try:
    try:
        df = pd.read_csv('data/Cigar.csv')
        print('Cargado desde data/Cigar.csv')
    except Exception:
        df = pd.read_excel('data/Cigar.xlsx', engine='openpyxl')
        print('Cargado desde data/Cigar.xlsx')
except Exception:
    try:
