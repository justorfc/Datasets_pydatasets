```python
# Código Colab para dataset: Soils
...
try:
    try:
        df = pd.read_csv('data/Soils.csv')
        print('Cargado desde data/Soils.csv')
    except Exception:
        df = pd.read_excel('data/Soils.xlsx', engine='openpyxl')
        print('Cargado desde data/Soils.xlsx')
except Exception:
    try:
