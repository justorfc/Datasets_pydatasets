```python
# Código Colab para dataset: Gleser
...
try:
    try:
        df = pd.read_csv('data/Gleser.csv')
        print('Cargado desde data/Gleser.csv')
    except Exception:
        df = pd.read_excel('data/Gleser.xlsx', engine='openpyxl')
        print('Cargado desde data/Gleser.xlsx')
except Exception:
    try:
