```python
# Código Colab para dataset: flchain
...
try:
    try:
        df = pd.read_csv('data/flchain.csv')
        print('Cargado desde data/flchain.csv')
    except Exception:
        df = pd.read_excel('data/flchain.xlsx', engine='openpyxl')
        print('Cargado desde data/flchain.xlsx')
except Exception:
    try:
