```python
# Código Colab para dataset: crohn
...
try:
    try:
        df = pd.read_csv('data/crohn.csv')
        print('Cargado desde data/crohn.csv')
    except Exception:
        df = pd.read_excel('data/crohn.xlsx', engine='openpyxl')
        print('Cargado desde data/crohn.xlsx')
except Exception:
    try:
