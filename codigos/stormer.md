```python
# Código Colab para dataset: stormer
...
try:
    try:
        df = pd.read_csv('data/stormer.csv')
        print('Cargado desde data/stormer.csv')
    except Exception:
        df = pd.read_excel('data/stormer.xlsx', engine='openpyxl')
        print('Cargado desde data/stormer.xlsx')
except Exception:
    try:
