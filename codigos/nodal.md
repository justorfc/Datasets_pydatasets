```python
# Código Colab para dataset: nodal
...
try:
    try:
        df = pd.read_csv('data/nodal.csv')
        print('Cargado desde data/nodal.csv')
    except Exception:
        df = pd.read_excel('data/nodal.xlsx', engine='openpyxl')
        print('Cargado desde data/nodal.xlsx')
except Exception:
    try:
