```python
# Código Colab para dataset: drughiv
...
try:
    try:
        df = pd.read_csv('data/drughiv.csv')
        print('Cargado desde data/drughiv.csv')
    except Exception:
        df = pd.read_excel('data/drughiv.xlsx', engine='openpyxl')
        print('Cargado desde data/drughiv.xlsx')
except Exception:
    try:
