```python
# Código Colab para dataset: melanoma
...
try:
    try:
        df = pd.read_csv('data/melanoma.csv')
        print('Cargado desde data/melanoma.csv')
    except Exception:
        df = pd.read_excel('data/melanoma.xlsx', engine='openpyxl')
        print('Cargado desde data/melanoma.xlsx')
except Exception:
    try:
