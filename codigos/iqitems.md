```python
# Código Colab para dataset: iqitems
...
try:
    try:
        df = pd.read_csv('data/iqitems.csv')
        print('Cargado desde data/iqitems.csv')
    except Exception:
        df = pd.read_excel('data/iqitems.xlsx', engine='openpyxl')
        print('Cargado desde data/iqitems.xlsx')
except Exception:
    try:
