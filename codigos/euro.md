```python
# Código Colab para dataset: euro
...
try:
    try:
        df = pd.read_csv('data/euro.csv')
        print('Cargado desde data/euro.csv')
    except Exception:
        df = pd.read_excel('data/euro.xlsx', engine='openpyxl')
        print('Cargado desde data/euro.xlsx')
except Exception:
    try:
