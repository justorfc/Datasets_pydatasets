```python
# Código Colab para dataset: bnct
...
try:
    try:
        df = pd.read_csv('data/bnct.csv')
        print('Cargado desde data/bnct.csv')
    except Exception:
        df = pd.read_excel('data/bnct.xlsx', engine='openpyxl')
        print('Cargado desde data/bnct.xlsx')
except Exception:
    try:
