```python
# Código Colab para dataset: Cavendish
...
try:
    try:
        df = pd.read_csv('data/Cavendish.csv')
        print('Cargado desde data/Cavendish.csv')
    except Exception:
        df = pd.read_excel('data/Cavendish.xlsx', engine='openpyxl')
        print('Cargado desde data/Cavendish.xlsx')
except Exception:
    try:
