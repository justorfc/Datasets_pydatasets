```python
# Código Colab para dataset: oats
...
try:
    try:
        df = pd.read_csv('data/oats.csv')
        print('Cargado desde data/oats.csv')
    except Exception:
        df = pd.read_excel('data/oats.xlsx', engine='openpyxl')
        print('Cargado desde data/oats.xlsx')
except Exception:
    try:
