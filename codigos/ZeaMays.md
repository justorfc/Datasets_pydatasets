```python
# Código Colab para dataset: ZeaMays
...
try:
    try:
        df = pd.read_csv('data/ZeaMays.csv')
        print('Cargado desde data/ZeaMays.csv')
    except Exception:
        df = pd.read_excel('data/ZeaMays.xlsx', engine='openpyxl')
        print('Cargado desde data/ZeaMays.xlsx')
except Exception:
    try:
