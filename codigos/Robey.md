```python
# Código Colab para dataset: Robey
...
try:
    try:
        df = pd.read_csv('data/Robey.csv')
        print('Cargado desde data/Robey.csv')
    except Exception:
        df = pd.read_excel('data/Robey.xlsx', engine='openpyxl')
        print('Cargado desde data/Robey.xlsx')
except Exception:
    try:
