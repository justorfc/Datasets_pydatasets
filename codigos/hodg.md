```python
# Código Colab para dataset: hodg
...
try:
    try:
        df = pd.read_csv('data/hodg.csv')
        print('Cargado desde data/hodg.csv')
    except Exception:
        df = pd.read_excel('data/hodg.xlsx', engine='openpyxl')
        print('Cargado desde data/hodg.xlsx')
except Exception:
    try:
