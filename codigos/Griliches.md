```python
# Código Colab para dataset: Griliches
...
try:
    try:
        df = pd.read_csv('data/Griliches.csv')
        print('Cargado desde data/Griliches.csv')
    except Exception:
        df = pd.read_excel('data/Griliches.xlsx', engine='openpyxl')
        print('Cargado desde data/Griliches.xlsx')
except Exception:
    try:
