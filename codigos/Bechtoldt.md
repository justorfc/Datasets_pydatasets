```python
# Código Colab para dataset: Bechtoldt
...
try:
    try:
        df = pd.read_csv('data/Bechtoldt.csv')
        print('Cargado desde data/Bechtoldt.csv')
    except Exception:
        df = pd.read_excel('data/Bechtoldt.xlsx', engine='openpyxl')
        print('Cargado desde data/Bechtoldt.xlsx')
except Exception:
    try:
