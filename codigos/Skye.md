```python
# Código Colab para dataset: Skye
...
try:
    try:
        df = pd.read_csv('data/Skye.csv')
        print('Cargado desde data/Skye.csv')
    except Exception:
        df = pd.read_excel('data/Skye.xlsx', engine='openpyxl')
        print('Cargado desde data/Skye.xlsx')
except Exception:
    try:
