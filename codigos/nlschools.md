```python
# Código Colab para dataset: nlschools
...
try:
    try:
        df = pd.read_csv('data/nlschools.csv')
        print('Cargado desde data/nlschools.csv')
    except Exception:
        df = pd.read_excel('data/nlschools.xlsx', engine='openpyxl')
        print('Cargado desde data/nlschools.xlsx')
except Exception:
    try:
