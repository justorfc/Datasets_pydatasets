```python
# Código Colab para dataset: cpus
...
try:
    try:
        df = pd.read_csv('data/cpus.csv')
        print('Cargado desde data/cpus.csv')
    except Exception:
        df = pd.read_excel('data/cpus.xlsx', engine='openpyxl')
        print('Cargado desde data/cpus.xlsx')
except Exception:
    try:
