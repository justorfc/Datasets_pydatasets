```python
# Código Colab para dataset: Lifeboats
...
try:
    try:
        df = pd.read_csv('data/Lifeboats.csv')
        print('Cargado desde data/Lifeboats.csv')
    except Exception:
        df = pd.read_excel('data/Lifeboats.xlsx', engine='openpyxl')
        print('Cargado desde data/Lifeboats.xlsx')
except Exception:
    try:
