```python
# Código Colab para dataset: cbpp
...
try:
    try:
        df = pd.read_csv('data/cbpp.csv')
        print('Cargado desde data/cbpp.csv')
    except Exception:
        df = pd.read_excel('data/cbpp.xlsx', engine='openpyxl')
        print('Cargado desde data/cbpp.xlsx')
except Exception:
    try:
