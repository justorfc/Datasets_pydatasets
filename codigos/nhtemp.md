```python
# Código Colab para dataset: nhtemp
...
try:
    try:
        df = pd.read_csv('data/nhtemp.csv')
        print('Cargado desde data/nhtemp.csv')
    except Exception:
        df = pd.read_excel('data/nhtemp.xlsx', engine='openpyxl')
        print('Cargado desde data/nhtemp.xlsx')
except Exception:
    try:
