```python
# Código Colab para dataset: ohio
...
try:
    try:
        df = pd.read_csv('data/ohio.csv')
        print('Cargado desde data/ohio.csv')
    except Exception:
        df = pd.read_excel('data/ohio.xlsx', engine='openpyxl')
        print('Cargado desde data/ohio.xlsx')
except Exception:
    try:
