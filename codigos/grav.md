```python
# Código Colab para dataset: grav
...
try:
    try:
        df = pd.read_csv('data/grav.csv')
        print('Cargado desde data/grav.csv')
    except Exception:
        df = pd.read_excel('data/grav.xlsx', engine='openpyxl')
        print('Cargado desde data/grav.xlsx')
except Exception:
    try:
