```python
# Código Colab para dataset: kootenay
...
try:
    try:
        df = pd.read_csv('data/kootenay.csv')
        print('Cargado desde data/kootenay.csv')
    except Exception:
        df = pd.read_excel('data/kootenay.xlsx', engine='openpyxl')
        print('Cargado desde data/kootenay.xlsx')
except Exception:
    try:
