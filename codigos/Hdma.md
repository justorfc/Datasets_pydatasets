```python
# Código Colab para dataset: Hdma
...
try:
    try:
        df = pd.read_csv('data/Hdma.csv')
        print('Cargado desde data/Hdma.csv')
    except Exception:
        df = pd.read_excel('data/Hdma.xlsx', engine='openpyxl')
        print('Cargado desde data/Hdma.xlsx')
except Exception:
    try:
