```python
# Código Colab para dataset: epil
...
try:
    try:
        df = pd.read_csv('data/epil.csv')
        print('Cargado desde data/epil.csv')
    except Exception:
        df = pd.read_excel('data/epil.xlsx', engine='openpyxl')
        print('Cargado desde data/epil.xlsx')
except Exception:
    try:
