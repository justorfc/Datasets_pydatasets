```python
# Código Colab para dataset: Airq
...
try:
    try:
        df = pd.read_csv('data/Airq.csv')
        print('Cargado desde data/Airq.csv')
    except Exception:
        df = pd.read_excel('data/Airq.xlsx', engine='openpyxl')
        print('Cargado desde data/Airq.xlsx')
except Exception:
    try:
